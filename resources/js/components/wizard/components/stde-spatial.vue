...
<div class="m-4">
    <form>
        ...
        <div :class="processing ? 'disabled-clicks' : ''">
            ...
            <div class="row justify-content-center text-center m-3">
                <div class="w-100 w-md-80 w-lg-70 w-xxl-55">
                    <div class="row justify-content-center text-center mt-5">
                        <div class="">
                            <div class="me-3">Proportion of genes to test spatial models: <input type="number" class="text-end text-sm border border-1 rounded w-25 w-md-35 w-xxl-15" v-model="params.sp_topgenes"></div>
                            <input type="range" min="0" :max="1" step="0.1" class="w-100" v-model="params.sp_topgenes">
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="p-3 text-center mt-4">
            <send-job-button label="Run Spatial tests" :disabled="processing || !this.params.samples.length || !this.params.test_type.length || !this.params.annotation.length || !this.params.clusters.length" :project-id="project.id" job-name="STDiffSpatial" @started="spatial" @ongoing="processing = true" @completed="processCompleted" :project="project" ></send-job-button>
        </div>


        <div v-if="!processing && ('stdiff_s' in project.project_parameters)" class="p-3 text-center mt-4">
            <a :href="stdiff_s.base_url + 'stdiff_s_results.xlsx'" class="btn btn-sm btn-outline-info me-2" download>Excel results - All samples</a>
        </div>
        ...
        <div v-if="!processing && ('stdiff_s' in project.project_parameters)">
            <div>
                <ul class="nav nav-tabs" id="myTab" role="tablist">
                    <li v-for="(sample, index) in stdiff_s.samples" class="nav-item" role="presentation">
                        <button class="nav-link" :class="index === 0 ? 'active' : ''" :id="sample + 'spatial-tab'" data-bs-toggle="tab" :data-bs-target="'#' + sample + 'spatial'" type="button" role="tab" :aria-controls="sample + 'spatial'" :aria-selected="index === 0">{{ sample }}</button>
                    </li>
                </ul>

                <div class="tab-content mx-3" id="myTabContent">
                    <div v-for="(sample, index) in stdiff_s.samples" class="tab-pane fade min-vh-50" :class="index === 0 ? 'show active' : ''" :id="sample + 'spatial'" role="tabpanel" :aria-labelledby="sample + 'spatial-tab'">

                        <div class="mt-4">
                            <ul class="nav nav-tabs" id="stdiff_s_tabs" role="tablist">
                                <li class="nav-item" role="presentation">
                                    <button class="nav-link active" :id="'stdiff_s_' + sample + 'table' + '-tab'" data-bs-toggle="tab" :data-bs-target="'#stdiff_s_' + sample + 'table'" type="button" role="tab" :aria-controls="'stdiff_s_' + sample + 'table'" aria-selected="true">Table</button>
                                </li>
                                <li class="nav-item" role="presentation">
                                    <button class="nav-link" :id="'stdiff_s' + sample + 'volcano' + '-tab'" data-bs-toggle="tab" :data-bs-target="'#stdiff_s' + sample + 'volcano'" type="button" role="tab" :aria-controls="'stdiff_s' + sample + 'volcano'" aria-selected="false">Volcano</button>
                                </li>
                            </ul>
                            <div class="tab-content" id="stdiff_s_tabsContent">

                                <div class="tab-pane fade show active" :id="'stdiff_s_' + sample + 'table'" role="tabpanel" :aria-labelledby="'stdiff_s_' + sample + 'table' + '-tab'">
...
<!--                                        </vue3-easy-data-table>-->
                                ...
                                <div class="tab-pane fade" :id="'stdiff_s' + sample + 'volcano'" role="tabpanel" :aria-labelledby="'stdiff_s' + sample + 'volcano' + '-tab'">
    ...
    export default {
        ...
        data() {
            return {
                ...
                params: {
                    ...
                    pairwise: false,
                    annotation: '',
                    n_genes: (('pca_max_var_genes' in this.project.project_parameters) && this.project.project_parameters.pca_max_var_genes >= 10) ? 10 : ('pca_max_var_genes' in this.project.project_parameters) ? this.project.project_parameters.pca_max_var_genes/2 : 0,
                    sp_topgenes: 0.2,
                    clusters: []
        ...
        methods: {
            ...
            spatial() {
                ...
                axios.post(this.spatialUrl, parameters)
                    .then((response) => {
                    })
                    .catch((error) => {
            ...
            volcano_plots(sample) {
                if(!('volcano_plots' in this.stdiff_s) || !this.stdiff_s.volcano_plots.length) return [];
                return this.stdiff_s.volcano_plots.filter(p => {return p.includes(sample)});
            }
        },
